...
if ($acao == "incluir")
{
	...
	if ($cod != '')
	{
		...
		if (is_numeric($cod))
		{
			...
			$query_rs_carrinho = "SELECT * FROM tb_carrinho WHERE tb_carrinho.cod = '".$cod."'  AND tb_carrinho.sessao = '".session_id()."'";
			...
			if ($totalRows_rs_carrinho == 0)
			{
				...
				$query_rs_produto = "SELECT * from tb_produtos where cod = '".$cod."'";
				...
				if ($totalRows_rs_produto > 0)
				{
					...
					$add_sql = "INSERT INTO tb_carrinho (id, cod, nome, preco, qtd, sessao)
					VALUES
...
if ($acao == "excluir")
{
	...
	if ($cod != '')
	{
		...
		if (is_numeric($cod))
		{
			...
			$query_rs_car = "SELECT * FROM tb_carrinho WHERE cod = '".$cod."'  AND sessao = '".session_id()."'";
			...
			{
				$sql_carrinho_excluir = "DELETE FROM tb_carrinho WHERE cod = '".$cod."' AND sessao = '".session_id()."'";
		...
		if (is_array($quant))
		{
			...
			foreach($quant as $cod => $qtd)
			{
				...
				if(is_numeric($cod) && is_numeric($qtd))
				{
					...
					$sql_modifica = "UPDATE tb_carrinho SET qtd = 	'$qtd' WHERE  cod = '$cod' AND sessao = '".session_id()."'";
...
<table width="100%" border="1" cellspacing="0" cellpadding="0">
  ...
  <?
  $sql_meu_carrinho = "SELECT * FROM tb_carrinho WHERE  sessao = '".session_id()."' ORDER BY nome ASC";
